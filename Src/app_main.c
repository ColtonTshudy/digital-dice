...
void setup() {
    ...
    LCD_Init();
    TP_Init();
    GUI_DrawRect(0,0,100,100,WHITE);
    // BSP_LCD_Init();
...
/**
 * @brief Generate a random 32-bit value using the noise present on floating ADC pins 
 * and on-chip temperature measurement
 * 
 */
uint32_t getRandSeed() {
    // for(uint8_t cycles = 0; cycles < 240; cycles++){
    uint32_t seed = 0;
    ...
    while (offset < 32 / 4) {
        if (isADCFinished == 1) {
            isADCFinished = 0;
            uint8_t i = 0;
            while (i < 5 && offset < 32 / 4) {
                seed += (adc_buffer[i++] & 0x0f) << (4 * offset++);
            }
            if (offset < 32 / 4) {
                HAL_ADC_Start_DMA(&hadc1, (uint32_t *)adc_buffer, 5);
            }
