...
void LCD_IO_ReadCmd8MultipleData24to16(uint8_t Cmd, uint16_t *pData, uint32_t Size,
                                       uint32_t DummySize);
void LCD_IO_ReadCmd16MultipleData8(uint16_t Cmd, uint8_t *pData, uint32_t Size, uint32_t DummySize);
void LCD_IO_ReadCmd16MultipleData16(uint16_t Cmd, uint16_t *pData, uint32_t Size,
                                    uint32_t DummySize);
void LCD_IO_ReadCmd16MultipleData24to16(uint16_t Cmd, uint16_t *pData, uint32_t Size,
                                        uint32_t DummySize);
...
#define GPIOX_AFR_(a, b, c)                                                                        \
  GPIO##b->AFR[c >> 3] = (GPIO##b->AFR[c >> 3] & ~(0x0F << (4 * (c & 7)))) | (a << (4 * (c & 7)));
...
#define LcdCmdWrite8(cmd8)                                                                         \
  {                                                                                                \
    LCD_RS_CMD;                                                                                    \
    LcdWrite8(cmd8);                                                                               \
    LCD_RS_DATA;                                                                                   \
  }
#define LcdCmdWrite16(cmd16)                                                                       \
  {                                                                                                \
    LCD_RS_CMD;                                                                                    \
    LcdWrite16(cmd16);                                                                             \
    LCD_RS_DATA;                                                                                   \
...
#define DMAX_IFCRALL_LCD_DMA_TX                                                                    \
  {                                                                                                \
    DMAX_IFCR(LCD_DMA_TX) = DMAX_IFCR_CTCIF(LCD_DMA_TX) | DMAX_IFCR_CHTIF(LCD_DMA_TX) |            \
                            DMAX_IFCR_CTEIF(LCD_DMA_TX) | DMAX_IFCR_CDMEIF(LCD_DMA_TX) |           \
...
void LCD_IO_WriteMultiData8(uint8_t *pData, uint32_t Size, uint32_t dinc) {
  ...
  if (!dinc) {
    d8s = *pData;
    pData = &d8s;
    dmacr = DMAX_STREAMX(LCD_DMA_TX)->CR =
        DMA_SxCR_TCIE | (0 << DMA_SxCR_MSIZE_Pos) | (0 << DMA_SxCR_PSIZE_Pos) |
        (0 << DMA_SxCR_MINC_Pos) | (1 << DMA_SxCR_DIR_Pos) |
        (DMACHN(LCD_DMA_TX) << DMA_SxCR_CHSEL_Pos) | (DMAPRIORITY(LCD_DMA_TX) << DMA_SxCR_PL_Pos);
  } else
    dmacr = DMAX_STREAMX(LCD_DMA_TX)->CR =
        DMA_SxCR_TCIE | (0 << DMA_SxCR_MSIZE_Pos) | (0 << DMA_SxCR_PSIZE_Pos) |
...
void LCD_IO_WriteMultiData16(uint16_t *pData, uint32_t Size, uint32_t dinc) {
  ...
  if (!dinc) {
    d16s = *pData;
    pData = &d16s;
    dmacr = DMA_SxCR_TCIE | (1 << DMA_SxCR_MSIZE_Pos) | (1 << DMA_SxCR_PSIZE_Pos) |
            (0 << DMA_SxCR_MINC_Pos) | (1 << DMA_SxCR_DIR_Pos) |
...
#define DMAX_IFCRALL_LCD_DMA_RX                                                                    \
  {                                                                                                \
    DMAX_IFCR(LCD_DMA_RX) = DMAX_IFCR_CTCIF(LCD_DMA_RX) | DMAX_IFCR_CHTIF(LCD_DMA_RX) |            \
                            DMAX_IFCR_CTEIF(LCD_DMA_RX) | DMAX_IFCR_CDMEIF(LCD_DMA_RX) |           \
...
#if LCD_SPI_MODE == 1
    SPIX->CR1 =
        (SPIX->CR1 & ~SPI_CR1_BR) | ((LCD_SPI_SPD_WRITE << SPI_CR1_BR_Pos) | SPI_CR1_BIDIOE);
#elif LCD_SPI_MODE == 2
...
void LCD_IO_ReadMultiData8(uint8_t *pData, uint32_t Size) {
  uint32_t dmacr;
  dmacr = DMA_SxCR_TCIE | (0 << DMA_SxCR_MSIZE_Pos) | (0 << DMA_SxCR_PSIZE_Pos) | DMA_SxCR_MINC |
...
void LCD_IO_ReadMultiData16(uint16_t *pData, uint32_t Size) {
  uint32_t dmacr;
  dmacr = DMA_SxCR_TCIE | (1 << DMA_SxCR_MSIZE_Pos) | (1 << DMA_SxCR_PSIZE_Pos) | DMA_SxCR_MINC |
          (DMACHN(LCD_DMA_RX) << DMA_SxCR_CHSEL_Pos) |
          (DMAPRIORITY(LCD_DMA_RX) << DMA_SxCR_PL_Pos) | (0 << DMA_SxCR_MBURST_Pos);
...
void LCD_IO_ReadMultiData16to24(uint16_t *pData, uint32_t Size) {
  ...
  DMAX_STREAMX(LCD_DMA_RX)->CR = (0 << DMA_SxCR_MSIZE_Pos) | (0 << DMA_SxCR_PSIZE_Pos) |
                                 DMA_SxCR_MINC | (0 << DMA_SxCR_MBURST_Pos) |
                                 (DMACHN(LCD_DMA_RX) << DMA_SxCR_CHSEL_Pos) |
  ...
  while (Size) {
    if (ntdr_follower != DMAX_STREAMX(LCD_DMA_RX)->NDTR) {
      ...
      __NOP();
      __NOP();
      __NOP(); /* a small wait until the DMA transfer is definitely completed */
...
#define DMA1_CLOCK_RX 0
  ...
  /* GPIO, DMA Clocks */
  RCC->AHB1ENR |= GPIOX_CLOCK(LCD_RS) | GPIOX_CLOCK(LCD_CS) | GPIOX_CLOCK(LCDTS_SCK) |
                  GPIOX_CLOCK(LCDTS_MOSI) | GPIOX_CLOCK_LCD_RST | GPIOX_CLOCK_LCD_BL |
                  GPIOX_CLOCK(LCDTS_MISO) | GPIOX_CLOCK(TS_CS) | GPIOX_CLOCK_TS_IRQ |
...
#if LCD_SPI_MODE == 1
  /* Half duplex */
  SPIX->CR1 = SPI_CR1_CPHA | SPI_CR1_CPOL | SPI_CR1_MSTR | SPI_CR1_SSM | SPI_CR1_SSI |
              (LCD_SPI_SPD_WRITE << SPI_CR1_BR_Pos) | SPI_CR1_BIDIMODE | SPI_CR1_BIDIOE;
#else  /* #if LCD_SPI_MODE == 1 */
  /* TX or full duplex */
  SPIX->CR1 = SPI_CR1_CPHA | SPI_CR1_CPOL | SPI_CR1_MSTR | SPI_CR1_SSM | SPI_CR1_SSI |
              (LCD_SPI_SPD_WRITE << SPI_CR1_BR_Pos);
...
#if LCD_SPI_MODE == 0
__weak void LCD_IO_ReadCmd8MultipleData8(uint8_t Cmd, uint8_t *pData, uint32_t Size,
                                         uint32_t DummySize) {
}
__weak void LCD_IO_ReadCmd8MultipleData16(uint8_t Cmd, uint16_t *pData, uint32_t Size,
                                          uint32_t DummySize) {
}
__weak void LCD_IO_ReadCmd8MultipleData24to16(uint8_t Cmd, uint16_t *pData, uint32_t Size,
                                              uint32_t DummySize) {
}
__weak void LCD_IO_ReadCmd16MultipleData8(uint16_t Cmd, uint8_t *pData, uint32_t Size,
                                          uint32_t DummySize) {
}
__weak void LCD_IO_ReadCmd16MultipleData16(uint16_t Cmd, uint16_t *pData, uint32_t Size,
                                           uint32_t DummySize) {
}
__weak void LCD_IO_ReadCmd16MultipleData24to16(uint16_t Cmd, uint16_t *pData, uint32_t Size,
                                               uint32_t DummySize) {
}
#else
...
//-----------------------------------------------------------------------------
void LCD_IO_ReadCmd8MultipleData16(uint8_t Cmd, uint16_t *pData, uint32_t Size,
                                   uint32_t DummySize) {
  WaitForDmaEnd();
  LcdSpiMode8();
  LCD_CS_ON;
...
//-----------------------------------------------------------------------------
void LCD_IO_ReadCmd16MultipleData8(uint16_t Cmd, uint8_t *pData, uint32_t Size,
                                   uint32_t DummySize) {
  WaitForDmaEnd();
  LcdSpiMode16();
  LCD_CS_ON;
